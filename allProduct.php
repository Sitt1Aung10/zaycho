...
<body style="background-image: url(img/testHomeImg.webp);">
    ...
    foreach ($productKinds as $productKind) {
        ...
        p.price, p.discount, p.coupon, p.phone_number, p.address,p.created_at
 FROM $productKind p
 ...
 WHERE u.can_login = 1"; // Ensures only posts from active users are shown
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Check if the post is within the last 3 days
                $createdAt = new DateTime($row['created_at']);
                $currentDate = new DateTime();
                $interval = $currentDate->diff($createdAt);
                $isNew = $interval->days <= 3 ? true : false;
                ...
                // Display New label if post is within 3 days
                if ($isNew) {
                    echo '<span class="new-label">Recent</span>';
                }
...
<div class="paymentSuccess" style="color: green; display: none;">Payment Success</div>
 ...
 <div class="kpay payment-method">
  <h4>KBZ PAY</h4> 
  <p>Total Amount: <span class="totalPrice">0</span> kyats</p> <img src="img/kpayqr.png" width="80px"> <br> <label class="voucher" for="kpayVoucher" style="width:100%;display:inline-block;text-align:center;">Upload Voucher</label> <input type="file" id="kpayVoucher" name="kpayVoucher" style="display: none;"> <br> <button class="payBtn" style="width:100%;">Pay</button> </div> <div class="ayapay payment-method"> <h4>AYA PAY</h4> <p>Total Amount: <span class="totalPrice">0</span> kyats</p> <img src="img/kpayqr.png" width="80px"> <br> <label class="voucher" for="ayapayVoucher" style="width:100%;display:inline-block;text-align:center;">Upload Voucher</label> <input type="file" id="ayapayVoucher" name="ayapayVoucher" style="display: none;"> <br> <button class="payBtn" style="width:100%;">Pay</button> </div> <div class="wavepay payment-method"> <h4>WAVE PAY</h4> <p style='color:#000;'>Total Amount: <span class="totalPrice">0</span> kyats</p> <img src="img/kpayqr.png" width="80px"> <br> <label class="voucher" for="wavepayVoucher" style='width:100%;color:#000;border-color:#000;display:inline-block;text-align:center;'>Upload Voucher</label> <input type="file" id="wavepayVoucher" name="wavepayVoucher" style="display: none;"> <br> <button class="payBtn" style="color:#000;width:100%;">Pay</button> </div> <div class="uabpay payment-method"> <h4>UAB PAY</h4> <p>Total Amount: <span class="totalPrice">0</span> kyats</p> <img src="img/kpayqr.png" width="80px"> <br> <label class="voucher" for="uabpayVoucher" style="width:100%;display:inline-block;text-align:center;">Upload Voucher</label> <input type="file" id="uabpayVoucher" name="uabpayVoucher" style="display: none;"> <br> <button class="payBtn" style="width:100%;border-color:#fff;">Pay</button> </div>
    ...
    `;
                    ...
                    paymentBoxElement.querySelector('.cancelBtn').addEventListener('click', () => {
                        paymentBoxElement.style.right = '-100%'; // Hide the payment box
                    });
                    ...
                    paymentBoxElement.querySelectorAll('.payBtn').forEach(button => {
                        button.addEventListener('click', event => {
                            // Find the parent payment method container
                            const paymentMethod = button.closest('.payment-method');

                            // Grab the associated file input inside this container
                            const voucherInput = paymentMethod.querySelector("input[type='file']");
                            ...
                            fetch
                                ...
                                ('upload_voucher.php', {
                                    ...
                                    if (data.includes("Payment Success")) {
                                        ...
                                        console.log('Files:', voucherInput.files);
                                        alert('Payment Successful!');
