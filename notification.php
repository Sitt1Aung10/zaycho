...
<body>
    <div class="banner"> 
        <a href="#notification">Order Notifications</a>
        <a href="#confirmOrder">Confirmed Orders</a>
        <a href="#rejectOrders">Rejected Orders</a>
    </div>
    <div class="notiPage">
        <?php
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<form method="post" class="notification" id="notification"  action="confirm_order.php">';
                ...
                // Check if the order has already been rejected
                $rejectedQuery = "SELECT 1 FROM reject_orders WHERE post_id = ? AND buyer_username = ?";
                $rejectedStmt = $mysqli->prepare($rejectedQuery);
                $rejectedStmt->bind_param('is', $postId, $buyerUsername);
                $rejectedStmt->execute();
                $isRejected = $rejectedStmt->get_result()->num_rows > 0;

                // $buttonStateReject = $isRejected ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : '';
                // $buttonTextReject = $isRejected ? 'Order Rejected' : 'Reject Order';
                $rejectedStmt->close();

                // Set button states dynamically
                $confirmButtonState = $isConfirmed ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : '';
                $rejectButtonState = $isRejected ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : '';
                $confirmButtonText = $isConfirmed ? 'Order Confirmed' : 'Confirm Order';
                $rejectButtonText = $isRejected ? 'Order Rejected' : 'Reject Order';

                // Show or hide the buttons based on confirmation/rejection status
                $confirmButtonDisplay = $isConfirmed || $isRejected ? 'style="display:none;"' : ''; // Hide confirm if rejected or confirmed
                $rejectButtonDisplay = $isConfirmed || $isRejected ? 'style="display:none;"' : ''; // Hide reject if confirmed or rejected



                echo '<button type="submit" id="confirmOrder" name="action" value="confirm" class="confirmOrder" ' . $confirmButtonState . ' ' . $confirmButtonDisplay . '>' . $confirmButtonText . '</button>';
                echo '<button type="submit" name="action" value="reject" class="rejectOrder" ' . $rejectButtonState . ' ' . $rejectButtonDisplay . '>' . $rejectButtonText . '</button>';
                echo '</form>';
            }
        } else {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<div class="notification" id="rejectOrders">';
        ...
        // Query to fetch rejected orders for the logged-in buyer
        $query = "SELECT post_id, post_kind, about_product,owner_username, rejection_reason 
FROM reject_orders 
WHERE buyer_username = ? 
ORDER BY rejection_time DESC";
        ...
        // Check if rejection messages are found
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $postId = htmlspecialchars($row['post_id']);
                $postKind = htmlspecialchars($row['post_kind']);
                $aboutProduct = htmlspecialchars($row['about_product']);
                $message = htmlspecialchars($row['rejection_reason']);
                $username = htmlspecialchars($row['owner_username']);

                // Display the rejection message
                echo '<div class="notification">';
                echo '<h5>Order Rejection<br>Voucher</h5>';
                echo '<p><strong>Product Kind:</strong> ' . ucfirst($postKind) . '</p>';
                echo '<p><strong>Product Details:</strong> ' . $aboutProduct . '</p>';
                echo '<p><strong>Message:</strong> ' . $message . ' By '. $username .'</p>';
                echo '<p style="color:gold;">Contact Admin - 0997727707</p>';
                echo '</div>';
            }
        } else {
            echo '<p>No rejected orders available.</p>';
        }
