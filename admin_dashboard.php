...
<header>
    ...
    <div class="banner">
        <a href="#notification">Orders Notification</a>
        <a href="#orderConfirm">Confirmed Orders</a>
    ...
    foreach ($productKinds as $productKind) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                ...
                <td><img src='uploads/{$row['images']}' alt='Product Image' class='product-image'></td>
    ...
    <div id="orderContainer">   
        ...
        <table  id="voucher_table">
            ...
            <?php
        $query = "SELECT voucher_path FROM `voucher_table`";
        $result = $mysqli->query($query);

        while ($row = $result->fetch_assoc()) {
            echo "<tr> 
                <td><img style='width:100px;' src='{$row['voucher_path']}'></td>
            </tr>";
        }
            ?>
        </table>
...
<table class="product-table" id="rejectOrders">
    <tr>
        <th>ID</th>
        <th>Owner Username</th>
        <th>Buyer Username</th>
    ...
    <?php
    $query = "SELECT id, owner_username, buyer_username, post_id, post_Kind, about_product, rejection_time 
              FROM reject_orders ORDER BY rejection_time DESC";
    $result = $mysqli->query($query);

    while ($row = $result->fetch_assoc()) {
        echo "<tr>
        <td style='background-color:#FF6347;color:#fff;'>{$row['id']}</td>
        <td>{$row['owner_username']}</td>
        <td>{$row['buyer_username']}</td>
        <td>{$row['post_id']}</td>
        <td>{$row['post_Kind']}</td>
        <td>{$row['about_product']}</td>
        <td>{$row['rejection_time']}</td>
        </tr>";
    }
    ?>
    ...
    <h2>User Accounts</h2>
    <table class="user-table" id="userAccounts">
        <tr>
            <th>ID</th>
            <th>Name</th>
            <th>Email</th>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            <td>{$row['name']}</td>
            <td>{$row['email']}</td>
          <td><button class='inspect-btn' onclick='inspectAccount(\"{$row['name']}\")'>Inspect Account</button></td>
          </tr>";
        }
        ?>
    ...
    <script>
        function inspectAccount(name) {
           // Redirect to the inspect_account page with username as a query parameter
        window.location.href = 'inspectAccount.php?name=' + name;
        }
    </script>
</body>
