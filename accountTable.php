...
if (isset($_POST['submit'])) {
    ...
    if ($result) {
        ...
        if ($numRows > 0) {
            ...
            // Insert new user
            $stmt = $conn->prepare("INSERT INTO users (name, email, password,address, phonenumber , nrcText, nrcFile , can_login) VALUES (?, ?, ?,?, ? ,?,? , 1)");
            $stmt->bind_param("sssssss", $name, $email, $password,$phonenumber,$address, $nrcText, $nrcFile);
            $stmt->execute();
            $stmt->close();
