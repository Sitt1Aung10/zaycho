...
if (session_status() === PHP_SESSION_NONE) {
    ...
    // Query to get only spare parts posts
    $productKind = 'spare_parts';
    ...
    if ($result->num_rows > 0) {
        // Loop through each result and display the spare parts posts
        while ($row = $result->fetch_assoc()) {
            // Split the images string into an array if multiple images were uploaded
            $images = explode(',', $row['images']);

            echo '<div class="post">';
            echo '<h2>Spare Parts Post by ' . htmlspecialchars($row['username'] ?? '') . '</h2>';
            ...
            foreach ($images as $image) {
                if (!empty($image) && file_exists('uploads/' . $image)) {
                    echo '<img src="uploads/' . htmlspecialchars($image) . '" alt="Spare Parts Image" width="100">';
                } else {
        ...
        echo '<p>No spare parts posts found.</p>';
    }
